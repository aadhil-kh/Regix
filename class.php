      ...
      <div class="container-fluid mimin-wrapper">
          ...
          <!-- start: content -->
            <div id="content">
                ...
                <div class="col-md-12">
                    <div class="panel">
                        <div class="panel-heading"><h3><?php echo "Department : ".$dep." && year :".$year;?></h3></div>
                        <div class="panel-body">
                             <?php
                            try 
                            {
                                    $stmt= $pdo->prepare("select count(distinct section) from ".$regix_id."_department_list where department=? and year=?");
                                    $stmt->execute([$dep,$year]);
                                    $cnt=$stmt->fetchColumn();
                                    $stmt= $pdo->prepare("select distinct section from ".$regix_id."_department_list where department=? and year=?");
                                    $stmt->execute([$dep,$year]);
                                    for($i=0;$i<$cnt;$i++)
                                    { 
                                        if($i==0)
                                        ...
                                        $row=$stmt->fetch();
                                        $s='A';
                                        for($k=1;$k<$row['section'];$k++)
                                            $s++;
                                        echo "<a href='menu.php?dep=".$dep."&year=".$year."&section=".$s."'>
                                                <div class='outer col-md-3'><div class='animated hvr-sweep-to-right fadeInLeft col-md-12 divelement'>  
                                              <span><b>Section:";
                                        echo $s."</b></span></div></div></a>";
                                    }
                                    echo "</div>";
                            }
