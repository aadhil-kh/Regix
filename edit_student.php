...
<head>
 ...
 <body id="mimin" class="dashboard">
                ...
                <div class="panel">
                  <div class="panel-body">
                      <div class="col-md-6 col-sm-12">
                          <h3 class="animated fadeInLeft"><span class="fa fa-male"></span>Students List</h3>
                    </div>
                  </div>                    
                </div>
                <div class="col-md-12">
                    <div class="col-md-12">
                    <div class="panel">
                        <div class="panel-heading"><h3 style="display:inline-block">Edit Students</h3>
    ...
    <script src="asset/js/plugins/handsontable.full.min.js"></script>
	   <?php
                                $stm=$pdo->prepare("select register_number,name,dob,gender from ".$regix_id."_student_list where department=? and year=? and section=?");
                                ...
                                {
																	  $data_array[$i]=[];
																	  $data_array[$i][0]=$row['register_number'];
																	  $data_array[$i][1]=$row['name'];
																	  $data_array[$i][2]=$row['dob'];
																	  $data_array[$i][3]=$row['gender'];
																	  $i++;
                                }	
																}
    ...
    <script type="text/javascript">
        $(document).ready(function () {
					...
					var secc=<?php echo json_encode($sec)?>;
					var container = document.getElementById('schedule');
					  var fill_data = function () {
							if(fill==true)
								{
									return Handsontable.helper.createSpreadsheetData(5,4);
								}
							else{
								return data;
							}
					...
					var hot = new Handsontable(container, {
						data: fill_data(),
						rowHeaders: true,
						colHeaders: ['Register Number','Name','DOB','Gender'],
						contextMenu: ['row_above', 'row_below', 'remove_row'],
						stretchH: 'all'
					}); 
					$('#savechanges').click(function(){
						var mydat=hot.getData();
						$.post('save_edit_student.php',{mydat:mydat,department:dep,year:year,section:section},function(response){
							window.location.href="edit_student.php?dep="+dep+"&year="+year+"&section="+secc;
						});
					});
				});
     </script>
